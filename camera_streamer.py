"""
Camera Streamer - Runs on Jetson Nano.
Captures synchronized frames from 2 USB cameras and streams to a remote server.
All processing (depth estimation, stereo, web UI) happens on the server side.

Usage:
    python3 camera_streamer.py                     # uses config defaults
    python3 camera_streamer.py --server 192.168.2.10
    python3 camera_streamer.py --server 192.168.2.10 --port 9000
"""

import argparse
import cv2
import numpy as np
import os
import signal
import socket
import struct
import subprocess
import sys
import time


# --- Defaults (can override via args) ---
DEFAULT_SERVER_HOST = "192.168.2.10"
DEFAULT_SERVER_PORT = 9000
CAMERA_0_INDEX = 0
CAMERA_1_INDEX = 1
CAMERA_WIDTH = 640
CAMERA_HEIGHT = 480
CAMERA_FPS = 30
JPEG_QUALITY = 90  # High quality since server does the processing
RECONNECT_DELAY = 3  # seconds between reconnection attempts

# Try to import config for camera settings
try:
    import config
    CAMERA_0_INDEX = config.CAMERA_0_INDEX
    CAMERA_1_INDEX = config.CAMERA_1_INDEX
    CAMERA_WIDTH = config.CAMERA_WIDTH
    CAMERA_HEIGHT = config.CAMERA_HEIGHT
    CAMERA_FPS = config.CAMERA_FPS
    USE_GSTREAMER = getattr(config, "USE_GSTREAMER", False)
    CAMERA_EXPOSURE_AUTO = getattr(config, "CAMERA_EXPOSURE_AUTO", 3)
    CAMERA_GAIN = getattr(config, "CAMERA_GAIN", None)
except ImportError:
    USE_GSTREAMER = False
    CAMERA_EXPOSURE_AUTO = 3
    CAMERA_GAIN = None


def _configure_exposure(camera_index, name):
    """Configure camera exposure via v4l2-ctl."""
    device = f"/dev/video{camera_index}"
    controls = [
        ("exposure_auto", CAMERA_EXPOSURE_AUTO),
        ("white_balance_temperature_auto", 1),
    ]
    if CAMERA_EXPOSURE_AUTO == 1:
        expo_val = getattr(config, "CAMERA_EXPOSURE_VALUE", 300) if "config" in dir() else 300
        controls.append(("exposure_absolute", expo_val))
    if CAMERA_GAIN is not None:
        controls.append(("gain", CAMERA_GAIN))

    for ctrl_name, ctrl_val in controls:
        try:
            result = subprocess.run(
                ["v4l2-ctl", "-d", device, "--set-ctrl", f"{ctrl_name}={ctrl_val}"],
                capture_output=True, text=True, timeout=2,
            )
            if result.returncode == 0:
                print(f"  [{name}] v4l2: {ctrl_name}={ctrl_val}")
        except Exception:
            pass


def open_camera(index, name):
    """Open a USB camera."""
    if USE_GSTREAMER:
        pipeline = (
            f"v4l2src device=/dev/video{index} ! "
            f"image/jpeg, width={CAMERA_WIDTH}, height={CAMERA_HEIGHT}, framerate={CAMERA_FPS}/1 ! "
            f"jpegdec ! videoconvert ! video/x-raw, format=BGR ! "
            f"appsink drop=1 sync=false"
        )
        cap = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
    else:
        cap = cv2.VideoCapture(index, cv2.CAP_V4L2)
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, CAMERA_WIDTH)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, CAMERA_HEIGHT)
        cap.set(cv2.CAP_PROP_FPS, CAMERA_FPS)
        cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*"MJPG"))
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)

    if not cap.isOpened():
        raise RuntimeError(f"Cannot open camera {index}")

    w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = cap.get(cv2.CAP_PROP_FPS)
    print(f"  [{name}] Opened: index={index}, {w}x{h} @ {fps:.0f}fps")

    _configure_exposure(index, name)
    return cap


def send_frame_pair(sock, frame0, frame1, quality=JPEG_QUALITY):
    """
    Send a synchronized frame pair over TCP.
    Protocol: [4 bytes: size0][4 bytes: size1][jpeg0][jpeg1]
    """
    encode_param = [cv2.IMWRITE_JPEG_QUALITY, quality]
    _, jpg0 = cv2.imencode(".jpg", frame0, encode_param)
    _, jpg1 = cv2.imencode(".jpg", frame1, encode_param)

    data0 = jpg0.tobytes()
    data1 = jpg1.tobytes()

    # Send header (two 4-byte unsigned ints: sizes of each JPEG)
    header = struct.pack("!II", len(data0), len(data1))
    sock.sendall(header + data0 + data1)


def main():
    parser = argparse.ArgumentParser(description="Camera Streamer for Jetson Nano")
    parser.add_argument("--server", default=DEFAULT_SERVER_HOST, help="Server IP")
    parser.add_argument("--port", type=int, default=DEFAULT_SERVER_PORT, help="Server port")
    parser.add_argument("--quality", type=int, default=JPEG_QUALITY, help="JPEG quality (1-100)")
    args = parser.parse_args()

    print("=" * 50)
    print("  Camera Streamer - Jetson Nano")
    print("=" * 50)
    print(f"  Server: {args.server}:{args.port}")
    print(f"  Cameras: [{CAMERA_0_INDEX}, {CAMERA_1_INDEX}]")
    print(f"  Resolution: {CAMERA_WIDTH}x{CAMERA_HEIGHT} @ {CAMERA_FPS}fps")
    print(f"  JPEG quality: {args.quality}")
    print("=" * 50)

    # Open cameras
    print("\n[Streamer] Opening cameras...")
    cap0 = open_camera(CAMERA_0_INDEX, "Cam0")
    cap1 = open_camera(CAMERA_1_INDEX, "Cam1")

    # Flush buffers
    for _ in range(5):
        cap0.grab()
        cap1.grab()

    running = True

    def signal_handler(sig, frame):
        nonlocal running
        print("\n[Streamer] Shutting down...")
        running = False

    signal.signal(signal.SIGINT, signal_handler)
    signal.signal(signal.SIGTERM, signal_handler)

    print("[Streamer] Cameras ready. Connecting to server...")

    frame_count = 0
    fps_time = time.time()

    while running:
        # Connect to server (with reconnection)
        sock = None
        while running and sock is None:
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                sock.settimeout(5)
                sock.connect((args.server, args.port))
                # Send resolution info as first message
                info = struct.pack("!HH", CAMERA_WIDTH, CAMERA_HEIGHT)
                sock.sendall(info)
                print(f"[Streamer] Connected to {args.server}:{args.port}")
            except (ConnectionRefusedError, OSError, socket.timeout) as e:
                sock = None
                print(f"[Streamer] Cannot connect to server: {e}. Retrying in {RECONNECT_DELAY}s...")
                time.sleep(RECONNECT_DELAY)

        # Stream loop
        while running:
            # Synchronized grab
            ok0 = cap0.grab()
            ok1 = cap1.grab()
            if not (ok0 and ok1):
                time.sleep(0.001)
                continue

            ret0, frame0 = cap0.retrieve()
            ret1, frame1 = cap1.retrieve()
            if not (ret0 and ret1):
                continue

            try:
                send_frame_pair(sock, frame0, frame1, args.quality)
                frame_count += 1

                # Print FPS every 5 seconds
                now = time.time()
                if now - fps_time >= 5.0:
                    fps = frame_count / (now - fps_time)
                    print(f"[Streamer] Streaming at {fps:.1f} FPS")
                    frame_count = 0
                    fps_time = now

            except (BrokenPipeError, ConnectionResetError, OSError):
                print("[Streamer] Connection lost. Reconnecting...")
                try:
                    sock.close()
                except Exception:
                    pass
                sock = None
                break

    # Cleanup
    cap0.release()
    cap1.release()
    if sock:
        sock.close()
    print("[Streamer] Done.")


if __name__ == "__main__":
    main()
